...
5. **Si es incompleto, señalar exactamente qué parte(s) fallan y por qué** (citando el criterio de la rúbrica que no se cumple), y sugerir una corrección concreta. Si el valor correcto no se puede derivar del contexto dado, proponerlo marcado como `Asunción: ...` con justificación — el mismo criterio que usa `heuristica-inferencia.md` de `skill-generar-escenarios-calidad`. Si el valor correcto sí es conocido (por ejemplo, porque el usuario lo dio en otra parte del mensaje), no marcarlo como asunción.
