...
1. **Clasificar el/los atributo(s) de calidad implicados**, siguiendo estos sub-pasos en orden, sin saltarlos:
   ...
   2. **Si un solo atributo tiene evidencia clara** y el resto no tiene anclaje real en el texto (solo se podría "estirar" hasta ahí), elegir ese atributo y seguir con el paso 2. **Si el atributo elegido se confunde fácilmente con otro parecido** (ej. Modificabilidad vs. Interoperabilidad/Integrabilidad, Usabilidad vs. Portabilidad, Disponibilidad vs. Confiabilidad), agregar una línea explícita de por qué se descartó la lectura vecina y no solo por qué se aceptó la elegida — ej. "Modificabilidad y no Interoperabilidad, porque el texto describe la capacidad de absorber un cambio externo sin propagar impacto, no el intercambio de datos en sí". Esto hace auditable la clasificación en vez de dejarla como una etiqueta sin razonamiento visible.
...
3. **Completar las partes que el input no menciona explícitamente**, aplicando `docs/conocimiento/heuristica-inferencia.md`. **Ambiente, Artefacto y Medida de la respuesta** se pueden inferir marcando `Asunción: ...` con justificación breve ligada al contexto del sistema. **Fuente del estímulo, Estímulo y Respuesta nunca se inventan**: si no están explícitas ni se pueden derivar con confianza, preguntar ofreciendo 2-3 lecturas concretas en vez de asumir en silencio (ver `heuristica-inferencia.md`, sección de partes que no se infieren).
...
5. **Redactar el escenario completo en el formato de salida fijo**, siempre una tabla markdown de dos columnas `Parte | Valor` con exactamente estas 6 filas, en este orden: Fuente del estímulo, Estímulo, Artefacto, Entorno, Respuesta, Medida de la respuesta. Encabezar la salida con la marca **`[ESTADO: BORRADOR]`** — este escenario todavía no pasó por `chequear-completitud-escenario`, así que no puede tratarse como validado ni usarse directamente en `arbol-utilidad` hasta que reciba `[ESTADO: VALIDADO]` de esa skill. Después de la tabla, agregar opcionalmente la oración final que las une en un escenario legible. Este formato es el mismo que usan `../../../shared/template-6-partes-sei.md` y `tests-no-leer/` — no improvisar un formato distinto, porque `chequear-completitud-escenario` y `arbol-utilidad` esperan consumir escenarios en esta misma estructura.
